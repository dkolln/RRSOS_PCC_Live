...
- **Cheats: Base Building** (third tab; `BaseBuildingEngine`, `BaseBuildingService`, `BuildTemplateStore`, `Instruments/BaseBuilding.razor`).
  ...
  or demand: Spacesuits (all `Skin-##`, 20), Personal equipment (type Tool 1110 + Gear 1120 except blueprints, 33), Vehicle equipment (type Part 1210, 13), Blueprints (`BlueprintT#`, 3) and Terra tokens (filled to
  ...
  Container3 (80), never Container1 (15); with Container2 the token box is 35 x 5000, with Container3 it is 80 x 5000. `BuildBundle` / `BuildRecipe.Bundles` in the engine and service. Untested in the game: whether it takes
  hand-added suits, blueprints and tokens; recipes are `Families` in `BaseBuildingService`, and an id missing from the item table is missing from its row). Facts
