...
- **Cheats: Base Building** (third tab; `BaseBuildingEngine`, `BaseBuildingService`, `BuildTemplateStore`, `Instruments/BaseBuilding.razor`).
  ...
  and filtered with the next item of a recipe (Fish = `Fish1Eggs`..`Fish14Eggs` from the item table; also Frog (17 with the gold), Butterfly, Tree, Plant seeds = `Seed0`..`Seed6`, `Seed7Humble`,
  `Seed8Humble`, `SeedGold`, and Vegetables = `Vegetable0Growable`..`Vegetable3Growable` (Eggplant, Squash, Beans, Mushroom) then `CookCocoaGrowable` and
  `CookWheatGrowable`, and Larvae = `LarvaeBase1`..`LarvaeBase3`, `Bee1Larvae`, `SilkWorm`, and Petri dishes = `Mutagen1`..`Mutagen4`, `Bacteria1`, and Quartz crystals = Pulsar, Balzar, Magnetar, Quasar, Solar, Cosmic (`CosmicQuartz` was missing from the item table and
  was added here as "Cosmic Quartz"; RRSOS-PCC does not have it yet); Rods = material then its rod, in pairs (Iridium, Rod-iridium, Uranim, Rod-uranium, Alloy, Rod-alloy, Osmium, Rod-osmium; then PlasticPolymer/Rod-plastic and
  Minable-Tungsten/Rod-tungsten, which are in the game's lists but not the item table yet); **labels are assigned in reading order from the beacon looking down the
  row: nearest position first, and within a position LEFT chest then RIGHT chest** (left = (-dirZ, dirX); for a row going north that is +z, the west side), so
  "an item then its rod" puts the item on the left and the rod on the right (this replaced a right-first order, which only rows built earlier have); Ores = Iron, Silicon, Titanium, Magnesium, Cobalt, Aluminium (the gId; the owner says "aluminum"), Sulfur, Zeolite, Obsidian, then the other planets' ores Bauxite, Dolomite, Uraninite, Selenium, Phosphorus, Amber (added to the item table here as ore type 2110, named for their ids; RRSOS-PCC does not have them), then `ice` (the owner: call ice an ore) (Iridium/Uranim/Osmium are in Rods; the game also
  has Bauxite, Dolomite, Uraninite, Selenium, Phosphorus, Amber, not in the item table); Crafting materials (the owner did not name it; this is my name) = NitrogenCapsule1, MethanCapsule1, Bioplastic1, Algae1Seed, Phytoplankton1-3, Fertilizer1-2 (+3), RedPowder1, FusionEnergyCell,
  FabricBlue and SmartFabric, CircuitBoard1, RocketReactor (+2); Fuses (the cartridge first, in the game's order), Food and cooking (flour, chocolate, cookie, honey, animal food; the harvested crops are `Vegetable*Growable`, i.e. already ready to eat, and
  sit in chests 80 at a time, so they are in Vegetables); Vegetables lists each crop with its seed beside it (crop left, seed right: Vegetable0Growable, Vegetable0Seed, ... CookWheatGrowable, CookWheatSeed = 12), Drones, Essentials (astrofood, astrofood2, water, oxygen); Tree seeds now start with `TreeRoot` (Tree Bark); Petri dishes also hold `DNASequence` and
  `GeneticTrait`; Crafting materials has Silk after the fabrics; Crafting materials also ends with Keycard1, KeyCard2 (later world), Explosive; still uncovered: Flare, and about 35 ids from later worlds not in the item table; **Toxic and purification** (16: ToxicGoo, ToxicWater, Toxins, ToxicSpores, MicroPlastics; PurifiedWater, PurificationCapsule, PurificationGel, ChlorineCapsule1, PristineMushroom;
  AntiToxinsExplosive1-2, ToxicityMedecine, ToxicityAmmo, ToxicityAmmoPack, ToxicityMedecinePack); Crafting materials also ends with Flare and AnimalBones. **47 ids from later worlds were added to the item
  table named by their own gId (the owner: "just name them the gid for now")** with guessed types, in BOTH repos' `worldobjectdata.json` and RRSOS-PCC's `worldobjects.json` (plus `CosmicQuartz` and the six other-planet
  ores, which are named): rename them when their real names are known. **Stocked ("one of each") groups**: the recipe **Equipment and tokens** builds 5 chests, each with a title as its label and NO item filter
  or demand: Spacesuits (all `Skin-##`, 20), Personal equipment (type Tool 1110 + Gear 1120 except blueprints, 33), Vehicle equipment (type Part 1210, 13), Blueprints (a chest FILLED with `BlueprintT1` chips; see below) and Terra tokens (filled to
  capacity with `TerraTokens5000`); one of each item is written as a new item record. A recipe with a stocked chest needs a template whose chests hold the biggest bundle (33), so the page offers only Container2 (35) and
  Container3 (80), never Container1 (15); with Container2 the token box is 35 x 5000, with Container3 it is 80 x 5000. `BuildBundle` / `BuildRecipe.Bundles` in the engine and service. Tested in the game 2026-09-26: spacesuits fine. **Blueprints: the first version wrote BlueprintT1, T2, T3 once each and only one survived**: every blueprint item in every real save is a bare `BlueprintT1` (T2/T3 come from RRSOS-PCC's old table and the game drops them on load), and a chip with no `liGrps` unlocks the next locked building when claimed (`UnlockedGroupsHandler.DecodeBlueprintServerRpc`: a chip's `liGrps` names the building it unlocks; with none it takes `UnlockingHandler.GetUnlockableGroup()` from `unlockingData.tier1..10GroupToUnlock`), so one chip is needed per building still locked. **Final design (plugin 0.8.0 + dashboard, confirmed in the game 2026-09-26 "fixed blueprints")**: the plugin's `BlueprintReader` writes `%LOCALAPPDATA%\RRSOS-PCC-Live\blueprints.json` once per game session (`chip`, `tiers` = 44 groups behind blueprints, `messages` = 4 toxicity items that are not blueprints, `loot` = 31 groups that drop a linked chip when deconstructed: arcade machine, exercise bike, treadmill, pool table, flower pots, ivy, fireplace, vending machine, ...; see `GroupsHandler.GetAllGroups()` + `Group.GetLootRecipeOnDeconstruct()`). The Blueprints bundle is one chip per group in tiers + loot (75, messages excluded) that is NOT already in the save header's `unlockedGroups`, written as `{"id","gId":"BlueprintT1","liGrps":"<building>"}` (bundle item text `chip@building`); the bundle has `SkipUnlocked` and `Split`, so it spreads over as many chests as the template needs, titled "Blueprints (1 of 2)" (Container2: 35 + 26 with 61 chips; Container3: one chest). Caveat: `unlockedGroups` does not list unlocks by terraforming progress, so a building unlocked that way still gets a chip (harmless). Without `blueprints.json` (game never run with plugin 0.8.0) the chest falls back to bare `BlueprintT1` to capacity. The Nexus article's `Blueprint*` ids (BlueprintPod9xA etc.) are not the game's group ids and are not used. Apply refuses a stocked chest holding more items than its slots. Tested on a scratch copy (61 chips linked, distinct, none for unlocked buildings, undo proof byte for byte) and in the game. Recipes are `Families` in `BaseBuildingService`, and an id missing from the item table is missing from its row). Facts
...
Nothing is in progress (v0.8.2 ships the linked-blueprint chest and plugin 0.8.0; the game needs a full restart for the new plugin). Open ideas (none asked for yet): more spoken alerts (an
